...
export class GapiOneSignalClientService implements OneSignalClient {
    ...
    basicRequest(url: string, apiKey: string, method: 'PUT' | 'POST' | 'GET' | 'DELETE' , body): Promise<any> {
        var options = <any>{
            url: url,
            method: method
        };
    ...
    async sendNotification(notification: Notification): Promise<SendNotificationResponse> {
        ...
        if (this.apps && this.apps.length > 0) {
            ...
            return await this.basicRequest(this.API_URI + Constants.NOTIFICATIONS_PATH, this.userAuthKey, 'POST', postBody);
        }
        if (this.app) {
            postBody.app_id = this.app.appId;
            return await this.basicRequest(this.API_URI + Constants.NOTIFICATIONS_PATH, this.app.appAuthKey, 'POST', postBody);
        }
    ...
    async cancelNotification(notificationId: string): Promise<any> {
        ...
        return await this.basicRequest(notificationUri, this.app.appAuthKey, 'DELETE', null);
    }
    ...
    async viewNotification(notificationId: string): Promise<any> {
        ...
        var notificationUri = this.API_URI + Constants.NOTIFICATIONS_PATH + '/' + notificationId + '?app_id=' + this.app.appId;
        return await this.basicRequest(notificationUri, this.app.appAuthKey, 'GET', null);
    }

    async viewNotifications(query: { limit: number, offset: number }): Promise<any> {
        if (!this.app) {
            throw 'You must define an "app" object.'
        }
    ...
    async viewApps(callback): Promise<any> {
        if (!this.userAuthKey) {
            throw 'You must define "userAuthKey" on Client'
        }
        return await this.basicRequest(this.API_URI + Constants.APPS_PATH, this.userAuthKey, 'GET', null);
    }
    ...
    async viewApp(appId: string): Promise<any> {
        ...
        return await this.basicRequest(this.API_URI + Constants.APPS_PATH + '/' + appId, this.userAuthKey, 'GET', null);
    }
    ...
    async createApp(body): Promise<any> {
        ...
        return await this.basicRequest(this.API_URI + Constants.APPS_PATH, this.userAuthKey, 'POST', body);
    }
    ...
    async addDevice(body): Promise<any> {
        ...
        return await this.basicRequest(this.API_URI + Constants.DEVICES_PATH, this.app.appAuthKey, 'POST', body);
    }
    ...
    async editDevice(deviceId: string, body): Promise<any> {
        ...
        return await this.basicRequest(this.API_URI + Constants.DEVICES_PATH + '/' + deviceId, this.app.appAuthKey, 'PUT', body);
    }
    ...
    async trackOpen(notificationId: string, body): Promise<any> {
        ...
        return await this.basicRequest(this.API_URI + Constants.NOTIFICATIONS_PATH + '/' + notificationId, this.app.appAuthKey, 'PUT', body);
    }
